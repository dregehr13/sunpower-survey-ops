...
- **Weekly completion projection — `projectWeek()` in `lib/metrics.cjs`** (v2, 2026-09-02, replaced `projectWeekTotal`). The old model summed a show-rate for scheduled rows and `min(daysLeft/ct, 1)` for the rest — it saw only work already in the queue and read ~40% low every Monday, because ~40% of a week's completions are **walk-in** (start and finish inside one week, almost all Sales Rep self-surveys). Things not to undo:
  ...
  - **Forward slippage, two places.** Every forward term (pipeline hazard, walk-in rate, restOfToday) is fit on RESOLVED history — slipped appointments already moved, stalled rows already read as never-completing — so each runs ~15% optimistic forward. A **`SLIP = 0.85` haircut** on the three prices that in at source (12-wk: raw over-projection ~+17 → ~+10, no MAE cost). The **residual** is caught by a rolling self-backtest at the same weekday: `recentBias` (+ = still over), `point` subtracts it clamped ±30%, ≥3 weeks. Tested alternatives that did NOT beat this: a pace-anchor blend (raw × recent-weekly-rate) — no MAE gain, added its own noise; a bigger/smaller SLIP — 0.85 is where Mon/Wed/Fri raw bias converge. Shown in the breakdown as "Model adjustment"
  - **`cacheKey` (surfaces pass `DATA_TS`) memoises the ~150ms result** — it ignores the page filter, so a filter change must not recompute it. `_projCache` is module-level
  - **Validated on 10 weeks** (`docs/METRICS.md`): Wed read bias +3 / MAE 17, Fri +1 / MAE 6; Monday +13 / MAE 27 with most of that MAE irreducible (a week's walk-in is 15–60 and Monday can't tell which)
  - **The reported spread is a "usually within", not a confidence interval.** `band` = median `|corrected residual|` (`recentMed`, ~±11 on a decent Wed/Thu window), `bandWide` = p85. Doug's call: `±σ` read "48–80" on a 64-point week — a range whose top was near double its bottom — while half the recent weeks were inside ±10. Surfaces say **"~64 usually ±11"** and the modal adds "most within ±18; the odd week runs wider". The genuine error at a Wed read is ±15–20 and no statistical trick shrinks it — tested median/quantile/last-5/pace-anchor bands, none both narrow AND honest — so the fix was the framing, not the number
  - **Surfaced**: Current → **Projected total** hero cell opens `#projModal` (`_renderProjModal`) — the build, by resource, by day, an 8-week accuracy log, the model inputs. The Current **Completions by day** chart carries a dashed amber **Projected** line on the days still to come (its own hidden `y2` axis pinned to the bar axis' max so they align). Both read `proj.byDay` from the same result — one definition. compose has a **Huddle** mode (`renderHuddle` / `_huddleModel`) beside Monday recap / Daily / Team: **three sentences to read on the call** — yesterday (goal vs actual + verdict), today (target + what's booked), the week (projection + pace) — then a `<details>` breakdown. **"Yesterday's goal" = `byDay` for yesterday from a `projectWeek` run anchored two days back** (`asOfHour 8`): the number that would have been committed on yesterday's call, not a blank. Today's target and both goal bands (±max(2, 20%)) come off the same per-day view. "Copy the report" gives the three sentences as plain text. **restOfToday** (`lib/metrics.cjs`) is the fix for "one completion by 9am doesn't mean today is done": rows **booked for today** at `hazSched` (held out of `pipeline`) + the day's usual **walk-up** (weekday share × trailing-3wk rate − banked − booked), both scaled by the export hour. The modal + huddle show a `_missSpark` sparkline of per-week miss instead of the old accuracy table; `api/morning-card.js` returns `stats.projected`. compose's nav brand is now **Site Survey Ops** (`<title>` "Site Survey Ops — Staging") — it is a staging surface, not just an email tool. **compose's projection anchors on `DATA_TS`, not `getYesterday()`** — anchoring before the export drops export-day completions and disagrees with the dashboard. Renames: "walk-in" → **"New this week"** (same *week*, mostly same day), "recent slippage" → **"Model adjustment"**, "range" → **"likely"**. The old `buildShowRates` stays exported/tested but unused
- Derived analytics also live in `lib/metrics.cjs`: businessDays (weekend rule), buildSegmentAvgs/lookupSegmentAvg, buildWeekdayShape, bandFor (≤target/≤target+2 bands), trendLabel (TREND_BAND_AVG=0.1 dashboard avg-based, TREND_BAND_MED=0.3 compose median-based — two calculations on purpose)

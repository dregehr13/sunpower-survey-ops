...
- **Weekly completion projection — `projectWeek()` in `lib/metrics.cjs`** (v2, 2026-09-02, replaced `projectWeekTotal`). The old model summed a show-rate for scheduled rows and `min(daysLeft/ct, 1)` for the rest — it saw only work already in the queue and read ~40% low every Monday, because ~40% of a week's completions are **walk-in** (start and finish inside one week, almost all Sales Rep self-surveys). Things not to undo:
  ...
  - **Surfaced**: Current → **Projected total** hero cell opens `#projModal` (`_renderProjModal`) — the build, by resource, by day, an 8-week accuracy log, the model inputs. The Current **Completions by day** chart carries a dashed amber **Projected** line on the days still to come (its own hidden `y2` axis pinned to the bar axis' max so they align). Both read `proj.byDay` from the same result — one definition. compose has a **Huddle** mode (`renderHuddle`) beside Monday recap / Daily / Team: the live prep sheet — the four-line report scaffold with yesterday filled in, today's target (`point × weekday share`), the week build, an accuracy log, the standing talking points, and a plain-text "Copy for the call". **compose's projection anchors on the export date (`DATA_TS`), not `getYesterday()`** — the rest of the daily branch is "yesterday"-framed, but a projection anchored before the export drops export-day completions and disagrees with the page it's written from. The old `buildShowRates` stays exported/tested but unused

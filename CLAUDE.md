...
- **The WIP page's KPI cards and age bar follow the page filter**, not just the table. They read `wipFiltered()`; the SS ratio takes both numerator and denominator from `wipScoped()` so it never divides one population by another. Filtering to a region with no recent completions correctly shows "—", not a national figure
- **The nav badge beside WIP is the attention count** (`attnItems()`), not the open-queue total. It is styled as an alert pill and read 113 while the page's own toggle read 42

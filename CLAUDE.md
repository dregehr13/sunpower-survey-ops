...
- **Weekly completion projection — `projectWeek()` in `lib/metrics.cjs`** (v2, 2026-09-02, replaced `projectWeekTotal`). The old model summed a show-rate for scheduled rows and `min(daysLeft/ct, 1)` for the rest — it saw only work already in the queue and read ~40% low every Monday, because ~40% of a week's completions are **walk-in** (start and finish inside one week, almost all Sales Rep self-surveys). Things not to undo:
  ...
  - **Slippage correction.** A rolling self-backtest at the same weekday over the trailing window yields `recentBias` (+ = model over-projected); `point` subtracts it, clamped ±30%, only with ≥3 weeks. The pipeline hazard is fit on resolved history where a slipped appointment already had its `scheduled` date rewritten, so forward it reads a few high — this removes that structurally. Shown in the breakdown as "Recent slippage"

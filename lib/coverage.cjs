...
(function (root, factory) {
  ...
  // actions — a page that emits a paragraph per market is read once.
  const ACTIONS = {
  ...
  function marketAdvice(m) {
    ...
    if (!(recentPerWeek > 0)) {
      return { ...ACTIONS.DORMANT, reasons: ['no jobs in the last ' + RECENT_WEEKS + ' weeks'
    ...
    if (vol >= VOLATILE_SHARE) {
      ...
      if (recentPerWeek >= DEPLOY_WEEKLY) {
        ...
        return { ...ACTIONS.DEPLOY, reasons };
      }
      return { ...ACTIONS.VENDOR, reasons };
    }
  ...
  return { EARTH_MI, milesBetween, DEFAULT_RADIUS_MI, RADIUS_CHOICES, clusterByRadius,
    ...
    ACTIONS, HIRE_WEEKLY, DEPLOY_WEEKLY, VOLATILE_SHARE, COACH_DEFECT, COACH_MIN_N, RECENT_WEEKS, marketAdvice,

...
(function (root, factory) {
  ...
  const DEFAULT_SURVEYOR = {
    ...
    roadFactor: ROAD_FACTOR,    // straight-line miles → road miles
    daysAvailable: 5,
  };

  // The shipped numbers are DEFAULTS, not findings, and every capacity figure
  // on the Resource page scales with them — so they are editable in the app
  // (Settings → Resource model) rather than only in a commit.
  //
  // One assignment point. setModel() rewrites the base that every other
  // function in this file reads through surveyorConfig(), which is the same
  // discipline applyAnchor() follows for r.start: teaching each caller about a
  // second source is how figures on one page come to disagree with figures on
  // another. A per-surveyor override in roster.json still wins, because it is
  // passed as `partial` and merged last.
  let MODEL = { ...DEFAULT_SURVEYOR };
  function setModel(partial) {
    MODEL = { ...DEFAULT_SURVEYOR };
    Object.entries(partial || {}).forEach(([k, v]) => {
      if (v != null && v !== '' && isFinite(v)) MODEL[k] = Number(v);
    });
    return { ...MODEL };
  }
  function getModel() { return { ...MODEL }; }

  function surveyorConfig(partial) { return { ...MODEL, ...(partial || {}) }; }

  // Minutes to drive `mi` straight-line miles, one way.
  function driveMinutes(mi, cfg) {
    const c = surveyorConfig(cfg);
    if (mi == null) return null;
    return (mi * (c.roadFactor || ROAD_FACTOR)) / Math.max(c.avgSpeedMph, 1) * 60;
  }
  ...
  // Road miles driven in a day working a market at `oneWayMi`: out and back,
  // plus one hop between each pair of jobs. Straight-line distances multiplied
  // by the road factor exactly as driveMinutes() does — a mileage reimbursement
  // is paid on the miles actually driven, not on the crow-flight distance.
  //
  // Built on the EXACT daily job count for the same reason weeklyCapacity() is:
  // flooring here would drop the hop for the job that lands every other day.
  //
  // Geometry only. What a mile is worth is a payroll question and lives with
  // the other cost knobs on the Resource page, not in this file.
  function dailyMiles(oneWayMi, cfg) {
    const c = surveyorConfig(cfg);
    if (oneWayMi == null) return null;
    const jobs = jobsPerDayExact(oneWayMi, c);
    return (oneWayMi * 2 + Math.max(jobs - 1, 0) * c.hopMi) * (c.roadFactor || ROAD_FACTOR);
  }
  ...
  // ── Siting a base ────────────────────────────────────────────────────────
  // What would a surveyor based HERE be able to reach? Every other figure in
  // this file measures the bases that exist; this one prices a base that does
  // not, which is the question no surface could answer before — the roster is
  // hand-maintained and the only way to test a location was to edit it.
  //
  // Deliberately not an optimiser. Asking the code for the BEST location would
  // hand back a point in a field somewhere with no house, no airport and no
  // reason for a person to live there, dressed as a recommendation; the manager
  // knows which towns are hireable and this only has to price the ones they
  // name. Same rule the recommendations follow — make the case, don't decide.
  //
  // `markets` are market-shaped: { ll, recentPerWeek, outsourcedPerWeek, reach }
  // where `reach` is what the EXISTING roster can already do, so that a market
  // already inside somebody's day-trip range is counted as in range but not as
  // gained. A candidate that only re-covers Portland has captured nothing.
  function siteCapture(ll, markets, cfg) {
    const c = surveyorConfig(cfg);
    const inRange = [], gained = [];
    let perWeek = 0, outsourced = 0, wSum = 0, wMi = 0;
    (markets || []).forEach(m => {
      if (!m || !m.ll || !ll) return;
      const mi = milesBetween(ll, m.ll);
      if (!reachable(mi, c)) return;
      const e = { m, miles: mi };
      inRange.push(e);
      if (m.reach !== REACH.DAY) gained.push(e);
      perWeek += m.recentPerWeek || 0;
      outsourced += m.outsourcedPerWeek || 0;
      // The typical drive is weighted by where the work actually is. A plain
      // mean over markets lets nine dormant villages at the edge of the radius
      // outvote the one market carrying every job, and capacity is then quoted
      // for a drive nobody makes.
      wMi += mi * (m.recentPerWeek || 0); wSum += (m.recentPerWeek || 0);
    });
    inRange.sort((a, b) => a.miles - b.miles);
    const meanMi = wSum ? wMi / wSum
      : (inRange.length ? inRange.reduce((s, e) => s + e.miles, 0) / inRange.length : null);
    return {
      inRange, gained, perWeek, outsourcedPerWeek: outsourced, meanMi,
      capacity: meanMi == null ? null : weeklyCapacity(meanMi, c),
      dailyMi: meanMi == null ? null : dailyMiles(meanMi, c),
    };
  }
  ...
  return { EARTH_MI, milesBetween, DEFAULT_RADIUS_MI, RADIUS_CHOICES, clusterByRadius,
    ...
    centroid, spreadMi, ROAD_FACTOR, DEFAULT_SURVEYOR, surveyorConfig, setModel, getModel, siteCapture,
    driveMinutes, dailyMiles, jobsPerDayExact, jobsPerDay, dayBudget, weeklyCapacity, reachable };
});

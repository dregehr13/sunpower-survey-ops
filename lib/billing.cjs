...
(function (root, factory) {
  ...
  // `reviewedLines` is the manual-clearance registry (reviewed.json) — for a
  // charge someone checked against Salesforce by hand and confirmed real, when
  // the automatic matcher can't find it because Salesforce's own record is
  // broken in a way nobody can fix there (e.g. an address missing its house
  // number). It is keyed to the exact billing line, not an account, and it
  // asserts NOTHING about the survey itself — no resource, no dates, no
  // outcome — so a reviewed line still has no match and still falls into
  // OUTCOME_UNMATCHED. All it does is stand in for a person having already
  // looked, so the same charge doesn't ask to be checked again every load.
  function reviewedKey(l) {
    return [norm(l.name), norm(l.address), l.date || '', l.subtype || ''].join('|');
  }
  function reconcile(lines, surveyRows, epcRows, archiveRows, reviewedLines) {
    const index = indexSurveys(surveyRows);
    const archiveIndex = indexSurveys(archiveRows || []);
    const epcIndex = indexSurveys(epcRows || []);
    const reviewedIndex = new Map((reviewedLines || []).map(r => [reviewedKey(r), r]));
    ...
    out.forEach(l => {
      ...
      // `epcName` is which EPC — Cobalt, NLITN, etc — read straight off the
      // registry row (epc.json's own `epc` field). Not every entry carries
      // one: the original 160-account pull recorded a financier (the loan
      // company) but never which EPC installed, so those fall back to a
      // plain "external EPC" with no name.
      if (e.row) { l.match = e.row; l.score = e.score; l.matchSource = 'epc'; l.epc = true; l.epcName = e.row.epc || ''; return; }
      l.match = null; l.score = m.score;
      const rv = reviewedIndex.get(reviewedKey(l));
      if (rv) { l.reviewed = true; l.reviewNote = rv.note || ''; }
    });
    ...
    chron.forEach(l => {
      ...
      if (isWork(l)) {
        if (!l.match && !l.reviewed) l.flags.push('no_sf_match');
  ...
  return { VENDORS, vendor, DEFAULT_VENDOR, usd,
    ...
    reconcile, reviewedKey, byAccount, summarize, byFlag };
});

...
(function (root, factory) {
  ...
  const VENDORS = {
    radicl: {
      ...
      // Statement columns, matched case-insensitively against a trimmed header.
      columns: { name: 'name', address: 'address', date: 'mission date',
        type: 'type', subtype: 'sub type', units: 'credits', balance: 'running credit balance' },
  ...
  const EXCEPTIONS = [
    ...
    { k: 'own_defect_rebill', label: 'Rework on their own defect', sev: SEVERITY.HIGH,
      why: 'A return visit billed at the full first-visit rate, on a survey this vendor performed and that Salesforce attributes to the surveyor. Whether it is billable is a contract question — listed so it is a decision rather than a default.' },
    { k: 'duplicate_charge', label: 'Same charge billed twice', sev: SEVERITY.HIGH,
      why: 'The same account was billed the same charge type more than once. A genuine second visit months later is possible, so the earlier line is shown beside it.' },
    { k: 'cross_statement', label: 'Appears on two statements', sev: SEVERITY.HIGH,
      why: 'An identical charge — same account, type and date — imported from two different statements. Either the statements overlap or the charge was billed twice.' },
    { k: 'cleanup_of_other', label: "Cleanup of someone else's survey", sev: SEVERITY.INFO,
      why: 'A return to finish a survey another resource started, billed at the full first-visit rate. Not disputable — this is what an incomplete first survey costs, and the invoice is the only place it shows up.' },
    { k: 'repeat_visit', label: 'Second visit to the same account', sev: SEVERITY.INFO,
      why: 'More than one work charge on one account. Often a legitimate sequence — a first visit and a return — but it is what an account actually cost, which no single line shows.' },
    { k: 'travel_adder', label: 'Travel adder', sev: SEVERITY.INFO,
      why: 'A travel adder applied beyond the vendor’s free radius. Not an error — tracked because it is a quarter of the bill and it is the line an in-house surveyor based in the market deletes outright.' },
  ];

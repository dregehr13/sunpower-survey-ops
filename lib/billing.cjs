...
(function (root, factory) {
  ...
  const EXCEPTIONS = [
    ...
    { k: 'duplicate_charge', label: 'Same charge billed twice', sev: SEVERITY.HIGH,
      why: 'The same account billed the same charge type more than once. The earlier line is shown beside it — a genuine second visit months later looks the same.' },
    ...
    { k: 'archived_project', label: 'Project predates the export', sev: SEVERITY.INFO,
      why: 'Surveyed in the billing period but sold before the dashboard\u2019s reporting window, so the live export has no row for it. Matched to the pre-cutoff registry instead. The charge is ordinary; only the attribution needed help.' },
  ...
  function reconcile(lines, surveyRows, epcRows, archiveRows) {
    ...
    chron.forEach(l => {
      ...
      if (l.archived) l.flags.push('archived_project');
  ...
  function byOutcome(lines) {
    ...
    charges.forEach(l => {
      ...
      let e = m.get(k);
      if (!e) e = m.set(k, { status: k, lines: [], units: 0, visits: 0, accounts: new Set(),
    ...
    return [...m.values()].map(e => ({
      ...
      perSurvey: e.visits ? usd(e.units, e.vendor) / e.visits : null,

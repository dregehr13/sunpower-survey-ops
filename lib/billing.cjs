...
(function (root, factory) {
  ...
  const EXCEPTIONS = [
    ...
    { k: 'cross_statement', label: 'Re-billed on another statement', sev: SEVERITY.HIGH,
      why: 'The same account and charge type on two different statements, carrying DIFFERENT dates. The same charge on the same date is not flagged and is not even stored twice — statement periods overlap, so a charge is routinely re-reported, and the import keeps one copy. A moved date is the shape a double bill takes, because neither statement shows the other.' },
  ...
  function byOutcome(lines) {
    ...
    const m = new Map();
    charges.forEach(l => {
      const k = l.match ? (l.match.project_status || 'Unknown') : OUTCOME_UNMATCHED;
      let e = m.get(k);

...
const isoDate = v => {
  ...
  // SheetJS returns a real date cell as an Excel serial when read with
  // raw:true. The monthly remittances ship the date as text, the YTD export
  // ships it as a date cell, so both shapes have to work or every line on one
  // of them silently loses its date.
  if (typeof v === 'number' && Number.isFinite(v)) {
    if (v < 1 || v > 80000) return '';
...
function parseWorkbookLines(wb, vendorId) {
  ...
  for (let i = hdrRow + 1; i < grid.length; i++) {
    ...
    if (!name) continue;
    // A statement can carry running-total rows inside the data area
    // ("Beginning Balance (carryforward)", "Ending Balance"). They have a name
    // and a number, so they parse as charges and add phantom spend: the YTD
    // export's Ending Balance alone was worth $11,986. They summarise the
    // lines around them and are never a charge in their own right.
    if (/^(beginning|ending|opening|closing)\s+balance/i.test(name)) continue;
    lines.push({
      vendor: vendorId,
      name,

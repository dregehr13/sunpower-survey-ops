...
(function (root, factory) {
  ...
  const ALARM_FLOOR_MULT = 1.5;  // Monday floor vs its own trailing median
  ...
  // Open WIP once the Monday after weekEndISO has cleared the weekend buildup.
  // Robust to the Fri/Sat rhythm by construction — it is measured after the drain.
  function mondayFloor(rows, weekEndISO) {
  ...
  // Early alarm: the Monday floor sitting above 1.5x its baseline. This moved a
  ...
  return { DATA_CUTOFF, inScope, filterRows, normalizeName, isComplete, isWIP, wipAgeFrom,
    ...
    rollingClearance, mondayFloor, clearanceAlarm, floorAlarm, floorBaseline, ALARM_CLEARANCE, ALARM_FLOOR_MULT,

...
(function (root, factory) {
  ...
  // Sum of Gross Price (Opportunity: Gross Price) across a set of rows. Rows
  // with no price on file (null — the field predates a given export, or a row
  // slipped through with it unset) count as 0 rather than dropping out, same
  // as every other sum in this file.
  function grossValue(rows) { return rows.reduce((s, r) => s + (r.gross_price || 0), 0); }
  ...
  return { DATA_CUTOFF, inScope, filterRows, normalizeName, isComplete, isWIP, everCompleted, effectiveComplete, wipAgeFrom,
    ...
    wipOn, meanWipForWeek, avgWeeklyCompletions, lastCompleteWeekEnd, ssRatioForWeek, ssRatioLive, ssRatioBand, grossValue,

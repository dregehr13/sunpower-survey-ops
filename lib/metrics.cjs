...
(function (root, factory) {
  ...
  // A survey is complete once it records a Site Survey Complete date. That
  // date is a separate Salesforce field from the resurvey dates, and nothing
  // that happens to the project afterward un-does the work: a later resurvey,
  // a cancellation, the List field drifting off 'Complete' — the initial
  // survey still happened, on the day it happened. Resurvey work is tracked on
  // its own dates (resurvey_requested / resurvey_complete, and isOpenResurvey)
  // and never blocks or reopens the initial completion. `isComplete` and
  // `everCompleted` are the same test — both names stay because ~50 call sites
  // import each.
  //
  // Before 2026-09-09 isComplete also required List === 'Complete', which
  // flipped it false the moment a survey was reopened for resurvey — a past
  // week's completion count shrank retroactively as resurveys landed, and ~14
  // reopened plus ~140 completed-then-cancelled rows sat in the open queue.
  // Doug reports on initial surveys; the resurvey is a separate line.
  function isComplete(r) { return !!r.complete; }
  function everCompleted(r) { return !!r.complete; }

  // A List of 'Inactive' (task parked) or 'Not Required' (no survey needed) is
  // neither active work nor — on its own — a finished survey.
  const PARKED_LIST = r => r.list === 'Inactive' || r.list === 'Not Required';

  // Row scope: started on/after the cutoff, and either the initial survey is
  // complete (always counts — the department did the work whatever became of
  // the deal) or the project is still an active first visit. A non-complete
  // Change Order drops out of WIP until it resolves back to In Progress;
  // non-complete At-Risk / Canceled rows stay out so they never show as open
  // WIP; parked lists are neither.
  function inScope(r, cutoff) {
    cutoff = cutoff || DATA_CUTOFF;
    if (!(r.start >= cutoff)) return false;
    if (isComplete(r)) return true;
    if (PARKED_LIST(r)) return false;
    return r.project_status === 'In Progress';
  }
  ...
  // Open initial-survey WIP: started, initial survey not yet complete, not
  // parked. Open resurveys are NOT here — they carry a completion date, so
  // they are terminal for WIP purposes and tracked separately (isOpenResurvey).
  function isWIP(r) { return !!(r.start && !isComplete(r) && !PARKED_LIST(r)); }
  ...
  // A resurvey still outstanding — genuine open work. The `list !== 'Complete'`
  // clause is load-bearing: 18 rows carry a resurvey request with no Resurvey
  // Complete Date but have gone back to list 'Complete' — the resurvey was
  // resolved and the date field was never filled in. Testing only the dates
  // counts those as open. Parked lists ('Inactive' / 'Not Required') are out
  // for the same reason isWIP drops them.
  //
  // `project_status === 'In Progress'` is the same active-project gate isWIP's
  // inScope applies (2026-09-09). The 2026-08-26 report change brought every
  // cancelled project into the export, and once isComplete became terminal
  // those reopened-resurvey rows reach the scoped set: 78 of 98 apparent open
  // resurveys were on cancelled deals and 6 more on At-Risk — no work to do.
  const isOpenResurvey = r => !!(r.resurvey_requested && !r.resurvey_complete && r.list !== 'Complete' && !PARKED_LIST(r) && r.project_status === 'In Progress');

  // The WIP page's queue union: initial WIP still in scope, or an open
  // resurvey. isWIP and isOpenResurvey partition the union, so this is never
  // double-counted. Shared so a second surface (compose) never grows its own
  // copy of what "open" means here.
  const isOpenQueue = r => (isWIP(r) && inScope(r)) || isOpenResurvey(r);

  // ── WIP queue status ─────────────────────────────────────────────────────
  // What is this open survey waiting on? One entry per pill, shared by the
  // dashboard's WIP table/status bar and compose's escalation list so a
  // status reads identically wherever it is used. `sort` is the table's
  // display order; `bar`/`bg`/`fg` are the status-bar fill and the badge.
  // Scheduled/Past due carry the only judgement here (blue = neutral, red =
  // over threshold); the waiting-reasons are categories of waiting, not
  // severities, so they stay unsaturated except where the reason itself is
  // the problem.
  const QS = {
    scheduled:   {key:'scheduled',   label:'Scheduled',            bg:'#eef2f6', fg:'#4f7fae',     bar:'#4f7fae', sort:0},
    pastdue:     {key:'pastdue',     label:'Past due',             bg:'#f9e7e4', fg:'#b3402e',     bar:'#b3402e', sort:1},
    pendingPhoto:{key:'pendingPhoto',label:'Pending photo upload', bg:'#e9f0f0', fg:'#3d6566',     bar:'#5f8a8b', sort:2},
    radicl:      {key:'radicl',      label:'Radicl scheduling',    bg:'#eeecf7', fg:'#5c4d99',     bar:'#5c4d99', sort:3},
    missingUb:   {key:'missingUb',   label:'Missing UB',           bg:'#f7efd9', fg:'#8a6413',     bar:'#c0912e', sort:4},
    awaitingRep: {key:'awaitingRep', label:'Awaiting rep',         bg:'#eef2f6', fg:'#4f7fae',     bar:'#64748b', sort:5},
    awaitingCust:{key:'awaitingCust',label:'Awaiting customer',    bg:'#eef1f4', fg:'#5b6b7d',     bar:'#7c8ba0', sort:6},
    likelyCancel:{key:'likelyCancel',label:'Likely cancel',        bg:'#fdeee2', fg:'#a85a24',     bar:'#a85a24', sort:7},
    blocked:     {key:'blocked',     label:'Blocked',              bg:'#f1eef2', fg:'#6b5d75',     bar:'#8a7d94', sort:8},
    new:         {key:'new',         label:'New',                  bg:'#faf9f6', fg:'#6b665a',     bar:'#c5c1b5', sort:9},
    unscheduled: {key:'unscheduled', label:'Unclassified',         bg:'#f3f1ea', fg:'#6b665a',     bar:'#a9a390', sort:10},
  };
  const QS_ORDER = ['scheduled','pastdue','pendingPhoto','radicl','missingUb','awaitingRep','awaitingCust','likelyCancel','blocked','new','unscheduled'];
  // Everything from Radicl scheduling down is a survey with no booking.
  // Anchored on the key rather than a slice count: Pending photo upload sits
  // above it because that survey was performed and needs a file, not a
  // booking.
  const QS_UNSCHED = QS_ORDER.slice(QS_ORDER.indexOf('radicl'));
  // The statuses a rep (or the customer chasing the rep) can actually move —
  // what an escalation email to sales leadership is for. Everything else
  // (Scheduled, Radicl scheduling, Pending photo upload, Blocked, New,
  // Unclassified) is either already moving or not the rep's to fix.
  const QS_REP_ACTIONABLE = ['awaitingRep', 'missingUb', 'awaitingCust', 'likelyCancel'];

  const wipSchedDate = r => isOpenResurvey(r) ? r.resurvey_scheduled : (r.scheduled || r.resurvey_scheduled);

  // Returns one of QS above. Order of the tests is the priority order — a
  // Radicl job whose review says the utility bill is missing is blocked on
  // the bill, not on Radicl. `today` is the export date (or wall clock),
  // ISO-8601, so a stale export doesn't misread a past date as current.
  function wipQueueStatus(r, today) {
    const openRs = isOpenResurvey(r);
    let sched = wipSchedDate(r);
    ...
    // A row design has kicked back for rework (reopened_by_design, or List
    // Reopened) that has no open-resurvey object yet: its r.scheduled is the
    // ORIGINAL visit, weeks or months stale, and must not drive Scheduled /
    // Past due — the note is the current truth. Only a PAST date is dropped;
    // a future one on such a row is a genuine new booking.
    if (!openRs && (r.reopened_by_design === '1' || r.list === 'Reopened') && sched && sched < today) sched = '';
    // The rep performed the survey and the SF automation failed to attach the
    // photos, so the job is waiting on an internal upload, not on a visit —
    // and its booking date, where it has one, is the past visit that already
    // happened. Distinct from Awaiting rep's NO PHOTOS / MISSING PHOTOS,
    // which is the rep not having taken any.
    if (any(/PENDING PHOTO UPLOAD/)) return QS.pendingPhoto;
    // A rep chase — missing photos, or "schedule with field" — beats a STALE
    // (past) schedule date, which on a rep self-survey is only the rep's own
    // nominal visit date. Not a future booking: that stays Scheduled.
    if (sched && sched < today && (subj.includes('MISSING PHOTOS') || subj.includes('SCHEDULE WITH FIELD') || comm.includes('NO PHOTOS'))) return QS.awaitingRep;
    if (sched && sched >= today) return QS.scheduled;
    if (sched && sched < today) return QS.pastdue;
    // Unscheduled — classify by last review subject. The at-risk state is
    // also written only in the note: "the account is marked at risk and he
    // needs retentions to reactivate it" under a subject reading just "REP
    // EMAIL". Matched on the specific past-tense phrasing rather than a bare
    // "at risk" — a rep-chase note saying "the customer does not want to
    // cancel" must not read as likely-cancel.
    if (r.project_status === 'At-Risk' || subj.includes('AT RISK') || subj.includes('AT-RISK')
       || /RETENTIONS TO REACTIVATE|NEEDS? RETENTIONS|MARKED AT.?RISK|ACCOUNT IS (MARKED )?AT.?RISK/.test(comm)) return QS.likelyCancel;
    // Reviewers write both "MISSING UB" and "MISSING UTILITY BILL". The same
    // ask written as prose is tested lower down, below Awaiting rep.
    ...
    // used when a job is handed to Radicl for booking. A handoff to Radicl is
    // also written as prose in the note — "submitted their information to
    // radicl", "it was sent to radicl" — under subjects that say nothing
    // themselves.
    if (any(/RADICL (RE)?SCHEDULING/) || (subj.includes('INITIAL SITE SURVEY') && subj.includes('RADICL'))
       || any(/(SENT|SUBMITTED|ASSIGNED|OVER)[\s\S]{0,40}?TO RADICL/)) return QS.radicl;
    // A bare "ATTEMPT n" subject (not "REP ATTEMPT") whose note reads
    ...
    // bottleneck from the rep chase below. Tested before the rep catch, which
    // "ATTEMPT" alone would otherwise sweep them into.
    if (/^(RESCHEDULE NEEDED\s*-\s*)?ATTEMPT \d/.test(subj.trim()) && !subj.includes('REP ATTEMPT')
       && /EMAILED THE CUSTOMER|CUSTOMER IN AN ATTEMPT|ATTEMPT TO SCHEDULE THE SITE SURVEY/.test(subj + ' ' + comm)) return QS.awaitingCust;
    // "FOLLOW UP WITH REP 8/17" is the rep-chasing state spelled out.
    // "RESCHEDULE NEEDED" is the rep cancelling a booked visit; SS offers a
    // new date and waits on them. Tested after Radicl, so the same subject
    // ending in RADICL SCHEDULING WITH CUSTOMER stays with Radicl.
    if (subj.includes('ATTEMPT') || comm.includes('REP ATTEMPT') || subj.includes('HOLDING') || comm.includes('NO PHOTOS') || subj.includes('MISSING PHOTOS')
       || subj.includes('RESCHEDULE NEEDED')
       || any(/FOLLOW.?UP WITH (THE )?REP/) || any(/\bFUP WITH (THE )?REP/)) return QS.awaitingRep;
    // The bill asked for in prose, under a subject that says nothing about
    // it. Tested BELOW Awaiting rep on purpose: on a rep-chase note the bill
    // is one ask among several, and the row is waiting on the rep, not on
    // the bill. The gap is tempered against UPLOAD so resolved phrasing
    // ("NO LONGER NEEDED ... I uploaded the UB") drops out.
    if (any(/(MISSING|NEEDS?|WAITING ON|REQUIRES?)((?!UPLOAD)[\s\S]){0,40}?(UTILITY BILL|\bUB\b)/)) return QS.missingUb;
    // Parked on work outside Site Survey — a reroof, mostly. Nobody in SS can
    // move it and there is nothing to schedule until the outside work lands.
    if (any(/NEW ROOF|REROOF|RE-?ROOF|ROOF (IS )?BEING (INSTALLED|REPLACED)|WAITING ON .{0,15}ROOF/)) return QS.blocked;
    // A COMPLETE note on a row that is not complete describes work that has
    // already finished — the initial survey — and says nothing about what
    // the row is waiting on now. "RESURVEY REQUIRED" is the subject design
    // writes when it kicks a survey back before the resurvey object exists.
    if (/^(SITE SURVEY )?COMPLETE\b/.test(subj.trim()) || subj.trim().startsWith('RESURVEY REQUIRED')) return QS.new;
    if (!r.last_reviewed_date) return QS.new;
    // A review subject nobody has seen before must land somewhere visible
    // rather than be forced into a reason it does not belong to.
  ...
  function buildProjectionModel(allRows, asOfISO, opts) {
    ...
    // acc[key] = [hits, atRisk]; keys: `${res}|${band}|open`, `${res}|sched`, ALL variants
    // The projection is initial-surveys only — resurvey completions are a
    // separate line and never enter this pipeline (Doug's reporting basis).
    const acc = {};
    const bump = (k, hit) => { (acc[k] = acc[k] || [0, 0])[1]++; if (hit) acc[k][0]++; };
    allRows.forEach(r => {
      if (!r.start) return;
      hzWeeks.forEach(([mon, sun]) => {
        if (r.start > mon) return;
        const doneBefore = r.complete && r.complete < mon;
        ...
        const hit = r.complete && r.complete >= mon && r.complete <= sun;
    ...
    const gCyc = allRows.filter(r => r.start >= _isoAddDays(asOfISO, -(rateWeeks * 7 + 28)) && r.start < asOfISO)
      .map(r => r.complete ? _dayDiff(r.start, r.complete) : null).filter(x => x != null && x >= 0);
    const gMed = med(gCyc) || 4;
    const regBuckets = {};
    allRows.forEach(r => {
      if (!r.region || !r.complete) return;
  ...
  function projectWeek(allRows, weekStartISO, asOfISO, opts) {
    ...
    // Pipeline: open initial-survey WIP as of asOf. A row counts as open then
    // if its project is still In Progress now (Change Orders drop out of WIP)
    // or its initial survey completed after asOf — and it started, and had not
    // completed, by asOf. Resurveys are out — a separate line.
    const _active = r => r.project_status === 'In Progress';
    const wip = allRows.filter(r =>
      (_active(r) || (r.complete && r.complete > asOfISO))
      && r.start && r.start <= asOfISO
      && !(r.complete && r.complete <= asOfISO));
    const _schedToday = r => r.scheduled === asOfISO;

    // Rest of today — two pieces the export cannot see as done yet:
    //   • rows BOOKED for today that just are not marked complete: their
    //     resource's measured show-rate (~0.9), so a full appointment book
    //     is not thrown away just because the morning export is early;
    //   • the day's usual walk-up volume beyond that — today's weekday
    //     completion share × trailing-3wk rate, minus what is banked and
    //     minus the booked rows already counted.
    // Both scaled by how much of the working day is still ahead (`asOfHour`,
    // the export hour): a booked visit survives a late export better than a
    // walk-up, so it keeps a 0.35 floor. `remainShare` covers only the days
    // strictly after asOf, so nothing here is also in `pipeline` or `intake`.
    let restOfToday = 0, expectedToday = 0, todayActual = 0, todaySchedExp = 0, todayWalkExp = 0;
    let todayScheduled = 0;
    const byRes = {}; model.RES.forEach(x => byRes[x] = { pipeline: 0, intake: 0, soFar: 0, today: 0 });
    model.RES.forEach(x => { byRes[x].soFar = doneRows.filter(r => r.resource === x).length; });
    if (asOfISO >= weekStartISO && asOfISO <= weekEndISO && daysLeft > 0) {
      ...
      const bySched = {};
      wip.forEach(r => {
        if (!_schedToday(r)) return;
        todayScheduled++;
        const e = model.hazSched(r.resource || 'ALL') * Math.max(dayAhead, 0.35);
        todaySchedExp += e;
        bySched[r.resource] = (bySched[r.resource] || 0) + e;
      });
      todayWalkExp = Math.max(0, expectedToday - todayActual - todaySchedExp) * dayAhead;
      restOfToday = todaySchedExp + todayWalkExp;
      model.RES.forEach(res => {
        byRes[res].today = (bySched[res] || 0) + todayWalkExp * (model.walkInMix[res] || 0);
      });
    }

    let pipeline = 0, pipelineSched = 0;
    wip.forEach(r => {
      if (_schedToday(r)) return; // counted in restOfToday
    ...
    let intake = model.walkInPerWeek * remainShare;
    const byDayIntake = {};
    const remDays = [];
    for (let i = 0; i < 7; i++) {
    ...
    // byRes[res].today was attributed inline with restOfToday above.

    // FORWARD SLIPPAGE, priced in rather than corrected after. Every forward
    // term — the hazard curves, the walk-in run rate, the rest-of-today
    // estimate — is fit on RESOLVED history, where a slipped appointment
    // already had its date moved to the week it landed and a stalled row
    // already shows as never-completing. Run forward, each reads a touch
    // optimistic; across 12 weeks the three together over-project the
    // remaining days by ~15%. The haircut moves that from a big trailing
    // bias correction (~16) down to a small one (~10) and nudges MAE. It
    // scales the pieces in place so byResource / byDay stay consistent.
    const SLIP = 0.85;
    pipeline *= SLIP; pipelineSched *= SLIP; intake *= SLIP;
    restOfToday *= SLIP; todaySchedExp *= SLIP; todayWalkExp *= SLIP;
    model.RES.forEach(res => { byRes[res].pipeline *= SLIP; byRes[res].intake *= SLIP; byRes[res].today *= SLIP; });
    Object.keys(byDayIntake).forEach(d => { byDayIntake[d] = Math.round(byDayIntake[d] * SLIP * 10) / 10; });
    ...
    // Spread of the residuals ONCE the bias is removed — the part the point
    // cannot fix. Median and p85 of |corrected error|.
    const _cabs = resid.map(e => Math.abs(e - (recentBias || 0))).sort((a, b) => a - b);
    const recentMed = _cabs.length ? _cabs[Math.floor(_cabs.length / 2)] : null;
    const recentP85 = _cabs.length ? _cabs[Math.min(_cabs.length - 1, Math.floor(_cabs.length * 0.85))] : null;
    ...
    // `band` is the TYPICAL miss — the median corrected error, i.e. the width
    // that has held about half the recent weeks. It is reported as "usually
    // within", not as a confidence interval: the old ±σ read ~±16 on a point
    // of 64 (a range whose top was nearly double its bottom) when half of
    // recent weeks were actually inside ±10. `bandWide` (p85) is the "some
    // weeks run wider" figure the modal shows alongside.
    const band = (recentMed != null) ? Math.max(4, z * recentMed)
      : (sd != null ? Math.max(4, z * sd) : null);
    const bandWide = recentP85 != null ? Math.max((band || 4) + 2, Math.round(recentP85)) : null;
    ...
    const out = {
      ...
      todayActual, todayScheduled,
      todayScheduledExpected: Math.round(todaySchedExp * 10) / 10,
      todayWalkExpected: Math.round(todayWalkExp * 10) / 10,
      ...
      recentMed: recentMed != null ? Math.round(recentMed) : null,
      bandWide,
  ...
  // Both sides are INITIAL surveys only — the denominator counts initial
  // completions, and wipOn counts open initial WIP (a reopened row carries a
  // completion date, so it is out of both). Resurvey backlog is its own number.
  //
  ...
  // Open initial-survey WIP as of a date: started by then, initial survey not
  // complete by then. A reopened-for-resurvey row carries a completion date and
  // is NOT counted — resurvey backlog is tracked separately.
  ...
  // Revenue = Opportunity: Gross Price − the dealer fee taken off it (Doug's
  // rule, 2026-09-24: a dealer fee is a pass-through cost, not revenue SS
  // survey work is measured against). Never read gross_price alone for a
  // revenue figure — this is the one definition. Missing values (null — the
  // field predates a given export, or slipped through unset) count as 0
  // rather than dropping the row out, same as every other sum in this file.
  function revenueValue(rows) { return rows.reduce((s, r) => s + (r.gross_price || 0) - (r.dealer_fee || 0), 0); }
  ...
  return { DATA_CUTOFF, inScope, filterRows, normalizeName, isComplete, isWIP, everCompleted, effectiveComplete, wipAgeFrom,
    hasRepGrace, ssDaysOpen, inRepGrace, hasResurveySig, isResurveyDefect, isOpenResurvey, isOpenQueue,
    QS, QS_ORDER, QS_UNSCHED, QS_REP_ACTIONABLE, wipSchedDate, wipQueueStatus,
    RS_CATEGORIES, rsCategories, rsCatLabel, fpy, avg, med, pct,
    wipOn, meanWipForWeek, avgWeeklyCompletions, lastCompleteWeekEnd, ssRatioForWeek, ssRatioLive, ssRatioBand, revenueValue,

...
(function (root, factory) {
  ...
  function projectWeek(allRows, weekStartISO, asOfISO, opts) {
    ...
    // asOf, from the weekday completion shape. Scales the unscheduled pipeline
    // hazard (a full week ahead → 1.0; Friday → the Sat/Sun tail).
    ...
    if (model.spwrCap != null) {
      ...
      const spwrTotal = s.soFar + s.pipeline + s.intake;
      if (spwrTotal > model.spwrCap) capAdj = -(spwrTotal - model.spwrCap);
    }

    const raw = completedSoFar + pipeline + intake + capAdj;
    ...
    const band = sd != null ? Math.max(4, z * sd) : null;

    // Per-day view: actual on past days, the projected remainder spread across
    // the days still to come by the weekday completion shape. One definition,
    // read by both the breakdown modal and the Current "by day" chart.
    const byDay = [];
    {
      const rem = Math.max(0, point - completedSoFar);
      const fut = [];
      let futW = 0;
      for (let i = 0; i < 7; i++) {
        const d = _isoAddDays(weekStartISO, i);
        ...
        futW += w;
        fut.push({ date: d, isFuture, w,
          actual: allRows.reduce((n, r) => n + (everCompleted(r) && r.complete === d ? 1 : 0), 0) });
      }
      fut.forEach(x => byDay.push({
        date: x.date, actual: x.actual,
        projected: x.isFuture ? Math.round(rem * (futW > 0 ? x.w / futW : 0)) : null,
      }));
    }

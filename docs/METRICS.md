...
| Market action | `lib/coverage.cjs` `marketAdvice()` → Resource page | One of Absorb / Deploy / Hire / Coach / Leave with vendor / Gone quiet, from five facts: is the market live, can we reach it, is there enough work, is that work durable, who is doing it badly. `HIRE_WEEKLY` 10 sits near a full week's capacity because a fixed hire must be justified by work that arrives every week; `DEPLOY_WEEKLY` 4 is lower because a posting is reversible. `COACH_MIN_N` 10 completions — at 5, one defect is 20% and would name an office off noise |
| In-house cost per survey | `index.html` Resource *Build vs buy* | **Quoted as a break-even, never as a rate.** Loaded weekly cost ÷ surveys completed is meaningless without a throughput: at modelled capacity it reads $115 against the vendor's $390, a sevenfold flattery on a page whose first insight says 14% of that capacity is realised. Break-even — surveys a week per surveyor to match the vendor — is the only figure here independent of the capacity model. Currently 4.4/wk against an actual 2.0/wk, i.e. $841/survey, **above** the vendor. The in-house inputs are an estimate and the panel says so |
| In-house work per surveyor | `index.html` Resource *The team* | **Attributed to a place, never to a person.** Salesforce records the survey's resource, not which surveyor performed it. Nearest-base attribution was tried and gave Portland entirely to one of two surveyors eight miles apart, printing the other at 0.0 — indistinguishable from idle, and not something the data can say. Work is counted within each surveyor's own day radius instead; overlapping ranges are counted against both and marked *shared range* |

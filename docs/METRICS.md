...
| Cost of a resurvey | `index.html` Quality page | `avg(start → resurvey_complete on defects) − avg(ct_total of clean completions)`, median shown beneath. What one resurvey costs end to end: currently **+20.7d avg / +13d median** against a 4d target. Was `avg(ct_full) − avg(clean ct_total)` until 2026-08-17 and read +5.3d — `ct_full` is `ct_total + ct_resurvey` (parse-sf.js), which skips the wait between the survey closing and the resurvey being requested. That wait is most of the calendar cost, and omitting it also made this figure a near-duplicate of Resurvey cycle sitting one line below it |
| Time to flag | `index.html` Quality page | `complete → resurvey_requested` on defects, avg with median beneath. Currently 16.2d avg / 6d median. Not the survey team's clock, which is the point: it is the largest of the three hero figures and a Design/review question rather than a field one. The three add up — 3.2d survey + 16.2d flag + 6.0d resurvey ≈ the 24.6d end-to-end |
